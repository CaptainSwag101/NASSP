...
void TVSA::TimeStep(double simdt)
{
	...
	if (acpower1)
	{
		...
		//CMC in any case
		if (tvcenable) PitchServo1Position += 0.023725*RAD*(double)sat->scdu.GetErrorCounter();
	...
	if (acpower2)
	{
		...
		//CMC in any case
		if (tvcenable) PitchServo2Position += 0.023725*RAD*(double)sat->scdu.GetErrorCounter();
